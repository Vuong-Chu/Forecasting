...
data_with_missing_times <-data_with_missing_times[6:length(data_with_missing_times),] %>% group_by(date) %>% mutate_each(funs(ifelse(is.na(.),0,.)))
...
tsdata <- ts(tsdata, c(2016,12,19), frequency = 365)
...
forecast.data <- ts(c(result1$x,combination),c(2016,12,19), frequency = 365)
